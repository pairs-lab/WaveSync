...
def load_pymaf_model(weight_path):
    ...
    """
    global _pymaf_model_cache
    if _pymaf_model_cache is not None:
        return _pymaf_model_cache
    ...
    _pymaf_model_cache = pymaf_model
    return pymaf_model
...
def get_pp_args(device):
    ...
    """
    global _pp_args_cache
    if _pp_args_cache is not None:
        _pp_args_cache.device = device
        return _pp_args_cache
...
def infer_smpl_with_pymaf(images):
    ...
    # Convert joints2d to numpy array for returning
    joints2d_array = np.array(joints2d) if joints2d else np.array([])
    
    return {
        'pose': pred_pose,
        'joints2d': joints2d_array
    }